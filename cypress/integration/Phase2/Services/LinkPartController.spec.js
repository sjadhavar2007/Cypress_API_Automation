...
describe('Link Part Controller', function(){
    var baseUrl;
    var requestBody;
    var id;
    var id07;
    ...
    before(function(){
        cy.fixture('Phase2/Services/CommonUrl').then(function(data){
            baseUrl = data.baseUrl_LinkPartController;
        })
        cy.fixture('Phase2/Services/LinkPartControllerBody').then(function(data){
            requestBody = data
        })
    })

    describe('Get request Find_all_Link_Part_Controller', function(){
            ...
            cy.request({
                ...
                } else 
                {
                    for( let i=0 ; i<this.contentLength ; i++ )
                    {
                        ...
                        expect(res.body.response.content[i]).to.have.property('lotNumber')
                        expect(res.body.response.content[i]).to.have.property('model')
                    //    expect(res.body.response.content[i]).to.have.property('mustMiFlag')
                        ...
                        expect(res.body.response.content[i]).to.have.property('productionFrom')
                        expect(res.body.response.content[i]).to.have.property('productionTo')
                    }
                }
    ...
    describe('Post request Create_Link_Part_Controller', function(){
        it('TC04-Positive-Verify the response status code & response body parameters', function(){
            cy.request({
                ...
                body: {
                    "id": requestBody.id,
                    "lotNumber": requestBody.lotNumber,
                    ...
                    "productionFrom": requestBody.productionFrom,
                    "productionTo": requestBody.productionTo,
                  }
            }).then(function(res){
                expect(res.status).to.eq(201)
                id = res.body.response.id
                expect(res.body.response).to.have.property('id', id)
                expect(res.body.response).to.have.property('lotNumber', requestBody.lotNumber)
                ...
                expect(res.body.response).to.have.property('productionFrom', requestBody.productionFrom)
                expect(res.body.response).to.have.property('productionTo', requestBody.productionTo)
            })
        })
    ...
    describe('Get request Find_by_ID_Link_Part_Controller', function(){
        ...
        it('TC10-Negative-Verify the response status code is 403 or not, if send the request with invalid Link Part ID', function(){
            cy.request({
                method: 'GET',
                url: baseUrl+id+1,
    ...
    describe('Put Request Update_Link_Part_Controller', function(){
        ...
        it('TC14-Negative-Verify the response status code is 403 or not, if send the request with invalid Link Part ID', function(){
            cy.request({
                method: 'PUT',
                url: baseUrl+id+1,
                headers: {
